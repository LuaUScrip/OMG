...
local function createBind(option, parent)
	...
	local round = library:Create("ImageLabel", {
		Position = UDim2.new(1, -6, 0, 4),
		Size = UDim2.new(0, -textService:GetTextSize(text, 16, Enum.Font.Cartoon, Vector2.new(9e9, 9e9)).X - 16, 1, -10),
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(40, 40, 40),
	...
	inputService.InputBegan:Connect(function(input)
		...
		elseif binding then
			local key
			pcall(function()
				if not keyCheck(input.KeyCode, {}) then
					key = input.KeyCode
				end
			end)
			pcall(function()
				if keyCheck(input.UserInputType, {Enum.UserInputType.MouseButton1, Enum.UserInputType.MouseButton2, Enum.UserInputType.MouseButton3}) and not key then
	...
	function option:SetKey(key)
		...
		round.Size = UDim2.new(0, -textService:GetTextSize(bindinput.Text, 15, Enum.Font.Cartoon, Vector2.new(9e9, 9e9)).X - 16, 1, -10)	
	end
end
...
local function createList(option, parent)
	...
	function option:AddValue(value)
		...
		label.InputEnded:Connect(function(input)
			if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
				clicking = false
				tweenService:Create(label, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = labelInContact and Color3.fromRGB(20, 20, 20) or Color3.fromRGB(30, 30, 30)}):Play()
			end
			if input.UserInputType == Enum.UserInputType.MouseMovement then
	...
	function option:Close()
		...
		tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = inContact and Color3.fromRGB(60, 60, 60) or Color3.fromRGB(40, 40, 40)}):Play()
		tweenService:Create(self.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1, Position = UDim2.new(0, main.AbsolutePosition.X - 5, 0, main.AbsolutePosition.Y - 10)}):Play()
		for _,label in next, content:GetChildren() do
			if label:IsA"TextLabel" then
...
local function createColorPickerWindow(option)
	...
	local function createColorButton(text, position, callback)
		local button = library:Create("ImageLabel", {
			ZIndex = 3,
			Position = UDim2.new(1, -8, 0, position),
	...
	createColorButton("Reset", 92, function()
		previousColors = {originalColor}
		option:SetColor(originalColor)
	end)
	
	createColorButton("Undo", 112, function()
	...
	createColorButton("Set", 132, function()
		table.insert(previousColors, currentColor)
		option:SetColor(currentColor)
	end)
	
	local rainbowButton = createColorButton("Rainbow", 152, function() end)
	local rainbowText = rainbowButton:FindFirstChild("TextLabel")
	...
	rainbowButton.InputBegan:Connect(function(Input)
		if Input.UserInputType == ui or Input.UserInputType == Enum.UserInputType.Touch then
			...
			if rainbowEnabled then
				rainbowLoop = runService.Heartbeat:Connect(function()
					...
					rainbowText.TextColor3 = chromaColor
				end)
			else
		...
		if Input.UserInputType == Enum.UserInputType.MouseMovement and not dragging then
			tweenService:Create(rainbowButton, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(10, 10, 10)}):Play()
		end
	end)
	
	rainbowButton.InputEnded:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseMovement and not dragging then
			tweenService:Create(rainbowButton, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(20, 20, 20)}):Play()
		end
	end)
...
local function createColor(option, parent)
	...
	function option:Close()
		library.activePopup = nil
		self.open = false
		tweenService:Create(self.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1, Position = UDim2.new(0, self.main.AbsolutePosition.X - 5, 0, self.main.AbsolutePosition.Y - 10)}):Play()
...
local function addElementFunctions(parent)
	...
	if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
		if library.activePopup then
			if input.Position.X < library.activePopup.mainHolder.AbsolutePosition.X or input.Position.Y < library.activePopup.mainHolder.AbsolutePosition.Y then
				library.activePopup:Close()
			elseif input.Position.X > library.activePopup.mainHolder.AbsolutePosition.X + library.activePopup.mainHolder.AbsoluteSize.X or input.Position.Y > library.activePopup.mainHolder.AbsolutePosition.Y + library.activePopup.mainHolder.AbsoluteSize.Y then
				library.activePopup:Close()
			end
		end
	end
